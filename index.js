...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Sentiremos saudades 😭😭, zuera se foda 🤡🤡👌 @${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: 'Calma ae'
				success: 'Ta ai camarada'
				error: {
					stick: 'So Fotos 😡😡',
					Iv: 'Link Invalido 😔👌'
				},
				only: {
					group: 'Esse comando é exclusivo para grupos amigão❌',
					ownerG:'Comando Exclusivo para Adms',
					ownerB: 'exclusivo para o dono do Bot,
					admin: 'exclusivo para o dono do Bot',
					Badmin: 'função so disponível quando o bot adquiri administração no grupo'
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["558481013787@s.whatsapp.net"] // COLOQUE SEU NÚMERO AQUI SEM ESPAÇOS
...
//AQUI NAO PRECISA MEXER
	...
	//AQUI NAO PRECISA MUDAR
				...
				case 'yt2mp3':
					if (args.length < 1) return reply('Não to vendo link nenhum 👁️')
				...
				case '':
					if (args.length < 1) return reply('Manda o link')
				...
				case 'stalk':
					try {
						if (args.length < 1) return client.sendMessage(from, 'cade o nome do mulek?''', text, {quoted: mek})
						...
						console.log(`Error :`, color(e,'red'))
						 reply ('Foi mal nome inválido 😞'')
					}
					break
			
				case 'nulis':
				case 'tulis':
					if (args.length < 1) return reply('Quer que eu escreva oque?')
					teks = body.slice(7)
					reply(mess.wait)
				...
				case 'tstiker':
				case 'tsticker':
					if (args.length < 1) return reply('cade o texto? marca pra mim pfv' ')
					ranp = getRandom('.png')
					rano = getRandom('.webp')
	...
	//AQUI NAO PRECISA MUDAR
			...
			case 'tagall':
				...
				case 'clearall':
					if (!isOwner) return reply('Quem foi que usou o comando? '') 
					anu = await client.chats.all()
					client.setMaxListeners(25)
					for (let _ of anu) {
						client.deleteChat(_.jid)
					}
					reply('Apagando em 3..2...1')
					break
				
				case 'bc':
					if (!isOwner) return reply('Quem é o retardado??') 
					...
					if (mentioned.length > 1) {
						teks = 'Você acaba de ser promovido, meus parabéns 🎉🎉'\n'
				...
				case 'demote':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Foi mal mas você foi rebaixado meu chapa 👌'\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
				...
				case 'add':
					...
					if (args[0].startsWith('08')) return reply('Escreve o número direto parça 🌞'')
					try {
						num = `${args[0].replace(/ /g, '')}@s.whatsapp.net`
						client.groupAdd(from, [num])
					} catch (e) {
						console.log('Error :', e)
						reply('Numero privado, foi mal mas esse não da pra adicionar 😞'' )
					}
					break
				...
				case 'kick':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Marca pra eu dar ban😈😈'')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Foi BAN te conhecer 🤝'\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`Foi BAN te conhecer 🤝 @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
//AQUI NAO PRECISA MUDAR		
...
//AQUI NAO PRECISA MUDAR                   
				...
				case 'toimg':
					if (!isQuotedSticker) return reply('❌ marca uma figurinha ai')
					reply(mess.wait)
